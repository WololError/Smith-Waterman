...
vector<char> read_all_bytes(const string& filename) {
    ...
    if (!file) throw runtime_error("Impossible de lire le fichier en memoire");
    ...
    if (file.read(buffer.data(), size))
        ...
        throw runtime_error("Erreur lors de la lecture du fichier binaire.");
    file.close();
...
string read_header_from_memory(const vector<char>& phr_data, const int a, const int b) {
    ...
    for (int i = a; i < b; i++) {
        ...
        if (phr_data[i] == 0x1A) { 
            int length = phr_data[i + 1];
            string text;
            for (int j = 0; j < length; j++) {
                text += static_cast<char>(phr_data[i + j + 2]);
            }
            header += text + " ";
            i += length + 2; 
        }
