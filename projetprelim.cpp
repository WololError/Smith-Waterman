...
int main(int argc, char** argv){
    ...
    //cout << pindata.numberOfprot << endl;
     ...
     for(int i = 0; i < pindata.sequence_offsets.size() - 1 ; i++){
         ...
         seq = read_sequence(psq, pindata.sequence_offsets[i], pindata.sequence_offsets[i + 1]);
         if (seq == query.sequence){
             //cout << "protein Trouvee !" << " i = " << i << endl;
             cout << read_header(phr,pindata.header_offsets[i],pindata.header_offsets[i + 1]) << endl;
